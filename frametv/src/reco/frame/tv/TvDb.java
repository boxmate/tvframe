...
import reco.frame.tv.db.sqlite.CursorUtils;
import reco.frame.tv.db.sqlite.DbModel;
import reco.frame.tv.db.sqlite.ManyToOneLazyLoader;
import reco.frame.tv.db.sqlite.OneToManyLazyLoader;
import reco.frame.tv.db.sqlite.SqlBuilder;
import reco.frame.tv.db.sqlite.SqlInfo;
...
public class TvDb {
	...
	private static final String TAG = "TvDb";

	private static HashMap<String, TvDb> daoMap = new HashMap<String, TvDb>();

	private SQLiteDatabase db;
	private DaoConfig config;

	private TvDb(DaoConfig config) {
		if (config == null)
	...
	private synchronized static TvDb getInstance(DaoConfig daoConfig) {
		TvDb dao = daoMap.get(daoConfig.getDbName());
		if (dao == null) {
			dao = new TvDb(daoConfig);
	 ...
	 * @param context
	 */
	public static TvDb create(Context context) {
	 ...
	 */
	public static TvDb create(Context context, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 *            数据库名称
	 */
	public static TvDb create(Context context, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 */
	public static TvDb create(Context context, String dbName, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
		config.setDebug(isDebug);
	 ...
	 *            数据库名称
	 */
	public static TvDb create(Context context, String targetDirectory,
			String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
	 ...
	 * @return
	 */
	public static TvDb create(DaoConfig daoConfig) {
	 ...
	 */
	public boolean insertBindId(Object entity) {
		checkTableExist(entity.getClass());
				...
				for (OneToMany one : ones) {
					...
					if (isFind) {
						List<?> list = findAllByWhere(one.getOneClass(),
                                one.getColumn() + "=" + id);
						if (list != null) {
							/* 濡�������OneToManyLazyLoader娉����锛������ц�������ユ�����杞芥�版�� */
	...
	public static class DaoConfig {
		...
		private String mDbName = "reco.db"; // 数据库名字
	...
	private SQLiteDatabase createDbFileOnSDCard(String sdcardPath,
		...
		if (!dbf.exists()) {
			try {
				...
				throw new DbException("��版��搴����浠跺��寤哄け璐�", ioex);
			}
		} else {
			return SQLiteDatabase.openOrCreateDatabase(dbf, null);
		}
