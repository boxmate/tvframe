...
import reco.frame.tv.http.RetryHandler;
import reco.frame.tv.http.SyncRequestHandler;
...
public class TvHttp {
  ...
  //------------------put 请求-----------------------
    ...
    public HttpHandler<File> download( String url,AjaxParams params, String target,boolean isResume, AjaxCallBack<File> callback) {
    	...
    	HttpHandler<File> handler = new HttpHandler<File>(appContext,httpClient, httpContext, callback,charset);
    ...
    protected <T> void sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, AjaxCallBack<T> ajaxCallBack) {
        ...
        new HttpHandler<T>(appContext,client, httpContext, ajaxCallBack,charset)
