...
public class TvBitmap {
	...
	private TvBitmap(Context context) {
		...
		configDiskCachePath(Utils.getDiskCacheDir(context, "afinalCache").getAbsolutePath());//锟斤拷锟界??锟界??锟界??锟界?猴拷瀵帮拷
		configDisplayer(new SimpleDisplayer());//锟斤拷锟界??锟斤拷锟藉?с??锟斤拷锟?
		configDownlader(new SimpleDownloader());//锟斤拷锟界??锟芥??锟芥?????锟斤拷
	}
		...
		if( ! mInit ){
			...
			bitmapLoadAndDisplayExecutor = Executors.newFixedThreadPool(mConfig.poolSize,new ThreadFactory() {
	            ...
	            public Thread newThread(Runnable r) {
					...
					// ?????х??缁捐法锟斤拷锟斤拷锟芥导锟斤拷锟斤拷缁狙?锟斤拷???锟界????????缁?锟斤拷锟斤拷锟斤拷锟芥い???锟斤拷锟斤拷??锟斤拷???锟界痪??锟斤拷???锟芥??锟介??锟斤拷锟姐??锟斤拷cpu锟斤拷??锟斤拷锟斤拷锟斤拷锟藉??锟界??锟斤拷婢讹拷???锟?
	...
	private void doDisplay(Button button, String uri, BitmapDisplayConfig displayConfig) {
		if(!mInit ){
			init();
		}
		...
		if (bitmap != null) {
			...
			final BitmapLoadAndDisplayTask task = new BitmapLoadAndDisplayTask(button, displayConfig );
			//?????х??姒?锟界?????锟藉?э拷锟?
			final AsyncDrawable asyncDrawable = new AsyncDrawable(mContext.getResources(), displayConfig.getLoadingBitmap(), task);
	...
	public void exitTasksEarly(boolean exitTasksEarly) {
		mExitTasksEarly = exitTasksEarly;
		if(exitTasksEarly)
			pauseWork(false);//??????锟斤拷锟斤拷锟斤拷锟斤拷缁捐法锟斤拷缂?锟斤拷锟斤拷
	}
	...
	public static boolean checkImageTask(Object data, View imageView) {
			...
			if (bitmapWorkerTask != null) {
				...
				if (bitmapData == null || !bitmapData.equals(data)) {
					...
					// 锟斤拷锟芥??锟芥??锟界痪璺?锟斤拷瀹歌??锟斤拷锟斤拷???锟窖?锟斤拷
					return false;
				}
			}
			return true;
	...
	private class BitmapLoadAndDisplayTask extends AsyncTask<Object, Void, Bitmap> {
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			// 锟斤拷???锟斤拷缁捐法锟斤拷锟斤拷锟借ぐ锟斤拷锟斤拷锟斤拷锟?imageview锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟藉??锟斤拷
	...
	private class TvBitmapConfig {
		public String cachePath;
		 ...
		 public TvBitmapConfig(Context context) {
				...
				//?????х??锟斤拷??э拷锟斤拷锟斤拷锟斤拷??с??锟斤拷锟芥径???????甸?╋拷锟芥?????锟斤拷楠?锟斤拷锟斤拷婢堆?锟斤拷,姒?锟界?????璐????锟介??锟界?硅??瀹筹拷锟斤拷1/2???锟?
